...
impl ConfigLocation {
    fn resolve(self, invocation_cwd: &Path) -> Result<PathBuf, ConfigDiscoveryError> {
...
pub fn load_config(location: ConfigLocation) -> Result<ConfigFile, ConfigLoadError> {
    let invocation_cwd =
        env::current_dir().map_err(|source| ConfigLoadError::CurrentDirectory { source })?;
    let path = location.resolve(&invocation_cwd)?;
    ...
    ConfigFile::new(config, config_dir, real_config_dir, invocation_cwd)
...
mod tests {
    ...
    fn dangling_symlink_is_present_but_cannot_be_canonicalized() {
        ...
        let load_error = load_config(ConfigLocation::Path(candidate.clone()))
